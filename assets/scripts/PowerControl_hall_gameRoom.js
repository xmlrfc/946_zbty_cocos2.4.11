...
var d = function (e) {
  ...
  o.prototype.start = function () {
    ...
    if (t) {
      var n = cc.sys.localStorage.getItem("energyResetDate");
      var i = new Date().toLocaleDateString();
      if (i != n) {
        if (Number(t) < 3) {
          window.energy = 3;
          cc.sys.localStorage.setItem("energy", "3");
        } else {
          window.energy = Number(t);
        }
        cc.sys.localStorage.setItem("energyResetDate", i);
      } else {
        window.energy = Number(t);
      }
    } else {
      ...
      cc.sys.localStorage.setItem("energyResetDate", new Date().toLocaleDateString());
    }
  };
  ...
  o.prototype.decreasePower = function (e) {
    if (!window.powerControl.decreasePowerFlag) {
      window.powerControl.decreasePowerFlag = true;
      setTimeout(function () {
        window.powerControl.decreasePowerFlag = false;
      }, 2000);
      if (window.energy >= 1) {
        window.energy -= 1;
        cc.sys.localStorage.setItem("energy", window.energy + "");
