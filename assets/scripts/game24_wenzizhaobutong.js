var n;
var i = this && this.__extends || (n = function (e, o) {
  return (n = Object.setPrototypeOf || {
    __proto__: []
  } instanceof Array && function (e, o) {
    e.__proto__ = o;
  } || function (e, o) {
    for (var t in o) {
      if (Object.prototype.hasOwnProperty.call(o, t)) {
        e[t] = o[t];
      }
    }
  })(e, o);
}, function (e, o) {
  function t() {
    this.constructor = e;
  }
  n(e, o);
  e.prototype = o === null ? Object.create(o) : (t.prototype = o.prototype, new t());
});
var r = this && this.__decorate || function (e, o, t, n) {
  var i;
  var r = arguments.length;
  var a = r < 3 ? o : n === null ? n = Object.getOwnPropertyDescriptor(o, t) : n;
  if (typeof Reflect == "object" && typeof Reflect.decorate == "function") {
    a = Reflect.decorate(e, o, t, n);
  } else {
    for (var c = e.length - 1; c >= 0; c--) {
      if (i = e[c]) {
        a = (r < 3 ? i(a) : r > 3 ? i(o, t, a) : i(o, t)) || a;
      }
    }
  }
  if (r > 3 && a) {
    Object.defineProperty(o, t, a);
  }
  return a;
};
var a = this && this.__awaiter || function (e, o, t, n) {
  return new (t || (t = Promise))(function (i, r) {
    function a(e) {
      try {
        l(n.next(e));
      } catch (o) {
        r(o);
      }
    }
    function c(e) {
      try {
        l(n.throw(e));
      } catch (o) {
        r(o);
      }
    }
    function l(e) {
      var o;
      if (e.done) {
        i(e.value);
      } else {
        (o = e.value, o instanceof t ? o : new t(function (e) {
          e(o);
        })).then(a, c);
      }
    }
    l((n = n.apply(e, o || [])).next());
  });
};
var c = this && this.__generator || function (e, o) {
  var t;
  var n;
  var i;
  var r;
  var a = {
    label: 0,
    sent: function () {
      if (1 & i[0]) {
        throw i[1];
      }
      return i[1];
    },
    trys: [],
    ops: []
  };
  r = {
    next: c(0),
    throw: c(1),
    return: c(2)
  };
  if (typeof Symbol == "function") {
    r[Symbol.iterator] = function () {
      return this;
    };
  }
  return r;
  function c(e) {
    return function (o) {
      return l([e, o]);
    };
  }
  function l(r) {
    if (t) {
      throw new TypeError("Generator is already executing.");
    }
    for (; a;) {
      try {
        t = 1;
        if (n && (i = 2 & r[0] ? n.return : r[0] ? n.throw || ((i = n.return) && i.call(n), 0) : n.next) && !(i = i.call(n, r[1])).done) {
          return i;
        }
        n = 0;
        if (i) {
          r = [2 & r[0], i.value];
        }
        switch (r[0]) {
          case 0:
          case 1:
            i = r;
            break;
          case 4:
            a.label++;
            return {
              value: r[1],
              done: false
            };
          case 5:
            a.label++;
            n = r[1];
            r = [0];
            continue;
          case 7:
            r = a.ops.pop();
            a.trys.pop();
            continue;
          default:
            if (!(i = (i = a.trys).length > 0 && i[i.length - 1]) && (r[0] === 6 || r[0] === 2)) {
              a = 0;
              continue;
            }
            if (r[0] === 3 && (!i || r[1] > i[0] && r[1] < i[3])) {
              a.label = r[1];
              break;
            }
            if (r[0] === 6 && a.label < i[1]) {
              a.label = i[1];
              i = r;
              break;
            }
            if (i && a.label < i[2]) {
              a.label = i[2];
              a.ops.push(r);
              break;
            }
            if (i[2]) {
              a.ops.pop();
            }
            a.trys.pop();
            continue;
        }
        r = o.call(e, a);
      } catch (c) {
        r = [6, c];
        n = 0;
      } finally {
        t = i = 0;
      }
    }
    if (5 & r[0]) {
      throw r[1];
    }
    return {
      value: r[0] ? r[1] : undefined,
      done: true
    };
  }
};
Object.defineProperty(exports, "__esModule", {
  value: true
});
var l = require("./game_wenzizhaobutong");
var d = cc._decorator;
var s = d.ccclass;
var u = d.property;
var h = function (e) {
  function o() {
    var o = e !== null && e.apply(this, arguments) || this;
    o.levelLabel = null;
    o.adNode = null;
    o.hintNode = null;
    o.hintNum = null;
    o.levelNode = null;
    o.answerNode = null;
    o.successNode = null;
    o.failNode = null;
    o.itemPre = null;
    o.clickClip = null;
    o.succClip = null;
    return o;
  }
  i(o, e);
  o.prototype.onLoad = function () {
    window.game = this;
    window.game24 = this;
    e.prototype.onLoad.call(this);
    this.init();
  };
  o.prototype.start = function () {
    this.loadConfig();
  };
  o.prototype.init = function () {
    this.successNode.active = false;
    this.failNode.active = false;
    this.levelLabel.string = "Lv." + window.currentLevel;
    this.position = [];
    this.answer = [];
    this.answerIndex = [];
    this.gameSuccFlag = null;
    this.gameFailFlag = null;
  };
  o.prototype.loadConfig = function () {
    return a(this, undefined, undefined, function () {
      var e;
      var o;
      var t;
      var n;
      var i;
      var r;
      var a;
      var l;
      return c(this, function (c) {
        switch (c.label) {
          case 0:
            if (window.levelResModel24_wenzizhaobutong) {
              return [3, 2];
            } else {
              e = window;
              return [4, window.adUtil.loadJsonByBundleSync("resources_wenzizhaobutong", "json/json" + window.model + "/level")];
            }
          case 1:
            e.levelResModel24_wenzizhaobutong = c.sent();
            c.label = 2;
          case 2:
            this.data = window.levelResModel24_wenzizhaobutong.json.data[window.currentLevel - 1];
            console.log(this.data);
            o = 0;
            for (; o < this.data.length; o++) {
              t = cc.instantiate(this.itemPre);
              if (this.position[o]) {
                t.x = this.position[o].x;
                t.y = this.position[o].y;
              } else {
                n = false;
                i = this.levelNode.children;
                n = false;
                i = this.levelNode.children;
                for (; !n;) {
                  n = true;
                  r = Math.random() * (this.levelNode.width - 100) - (this.levelNode.width - 100) / 2;
                  a = Math.random() * (this.levelNode.height - 100) - (this.levelNode.height - 100) / 2;
                  l = 0;
                  for (; l < i.length; l++) {
                    if (Math.abs(i[l].x - r) < 100 && Math.abs(i[l].y - a) < 100) {
                      n = false;
                      break;
                    }
                  }
                  if (n) {
                    t.x = r;
                    t.y = a;
                    this.position.push(cc.v2(r, a));
                  }
                }
              }
              t.getComponent("itemModel24_wenzizhaobutong").init(this.data[o], o);
              this.levelNode.addChild(t);
            }
            return [2];
        }
      });
    });
  };
  o.prototype.checkEnd = function () {
    var e = this;
    if (!this.gameSuccFlag && !this.gameFailFlag && this.answer.length == this.data.length) {
      for (var o = true, t = 0; t < this.data.length; t++) {
        if (this.data[t] != this.answer[t]) {
          o = false;
          break;
        }
      }
      if (o) {
        this.gameSuccFlag = true;
        this.successNode.active = true;
        window.adUtil.playMusicEffect(window.game.succClip, false);
        this.levelUpCallback();
        this.scheduleOnce(function () {
          e.judgeShowWinUI(6);
        }, 1);
      } else {
        this.gameFailFlag = true;
        this.failNode.active = true;
        this.scheduleOnce(function () {
          cc.assetManager.loadBundle("resources_wenzizhaobutong", function (e, o) {
            o.load("prefab/ui/failUI" + window.adUtil.useHallData("gameoverSkin", "wenzizhaobutong"), cc.Prefab, function (e, o) {
              var t = cc.instantiate(o);
              cc.find("Canvas").addChild(t);
              t.zIndex = 1;
            });
          });
        }, 1);
      }
    }
  };
  o.prototype.showTips = function () {
    var e = function () {
      cc.assetManager.loadBundle("resources_wenzizhaobutong", function (e, o) {
        o.load("prefab/ui/answer", cc.Prefab, function (e, o) {
          var t = cc.instantiate(o);
          cc.find("Canvas").addChild(t);
          t.zIndex = 1;
        });
      });
    };
    var o = Number(cc.sys.localStorage.getItem("hintNum_wenzizhaobutong"));
    if (o > 0) {
      o--;
      cc.sys.localStorage.setItem("hintNum_wenzizhaobutong", o + "");
      e();
      if (o <= 0) {
        this.adNode.active = true;
        this.hintNode.active = false;
      } else {
        this.hintNum.string = o + "";
      }
    } else {
      window.vivoVideoAd_hall.initRewardedAd(function () {
        e();
      }, function () {});
    }
  };
  r([u(cc.Label)], o.prototype, "levelLabel", undefined);
  r([u(cc.Node)], o.prototype, "adNode", undefined);
  r([u(cc.Node)], o.prototype, "hintNode", undefined);
  r([u(cc.Label)], o.prototype, "hintNum", undefined);
  r([u(cc.Node)], o.prototype, "levelNode", undefined);
  r([u(cc.Node)], o.prototype, "answerNode", undefined);
  r([u(cc.Node)], o.prototype, "successNode", undefined);
  r([u(cc.Node)], o.prototype, "failNode", undefined);
  r([u(cc.Prefab)], o.prototype, "itemPre", undefined);
  r([u(cc.AudioClip)], o.prototype, "clickClip", undefined);
  r([u(cc.AudioClip)], o.prototype, "succClip", undefined);
  return r([s], o);
}(l.default);
exports.default = h;